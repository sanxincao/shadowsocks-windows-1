...
namespace Shadowsocks.Controller
{
    ...
    public class GeositeUpdaterService : IService
    {
        private static readonly Logger logger = LogManager.GetCurrentClassLogger();

        private static readonly string DATABASE_PATH = PathUtil.GetTempPath("dlc.dat");
        ...
        public GeositeUpdaterService(Configuration configuration)
        {
        ...
        public void Startup()
        {
            ...
            {
                GEOSITE_DB = File.ReadAllBytes(DATABASE_PATH);
            }
            else
            {
                File.WriteAllBytes(DATABASE_PATH, Resource.DLC_DAT);
        ...
        {
            var geositeUrl = string.IsNullOrWhiteSpace(_configuration.geositeUrl) ? GEOSITE_URL : _configuration.geositeUrl;
            var blacklist = _configuration.geositePreferDirect;
            ...
            try
            {
                ...
                string remoteSHA256Sum = (await Utils.HttpClient.GetStringAsync(GEOSITE_SHA256SUM_URL)).Substring(0, 64).ToUpper();

                logger.Info($"Remote SHA256 sum: {remoteSHA256Sum}");
                ...
                logger.Info($"Local SHA256 sum: {localDBHash}");
                ...
                logger.Info($"Actual SHA256 sum: {downloadedDBHash}");
                ...
                using (FileStream fileStream = File.Create(DATABASE_PATH))
                    fileStream.Write(downloadedBytes, 0, downloadedBytes.Length);

                // update stuff
                GEOSITE_DB = downloadedBytes;
                LoadGeositeList();
                bool pacFileChanged = MergeAndWritePACFile(_configuration.geositeDirectGroups, _configuration.geositeProxiedGroups, blacklist);
...
{abpContent}";
    ...
    internal static class GeositeUpdaterServiceEx
    {
        private static GeositeUpdaterService service;
    }
}
