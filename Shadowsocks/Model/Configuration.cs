    ...
    public class Configuration
    {
        ...
        public static bool ValidateGeositeGroupList(List<string> groups)
        {
            foreach (var geositeGroup in groups)
                if (!GeositeUpdaterService.CheckGeositeGroup(geositeGroup)) // found invalid group
                {
#if DEBUG
                    logger.Debug($"Available groups:");
                    foreach (var group in GeositeUpdaterService.Geosites.Keys)
                        logger.Debug($"{group}");
#endif
        ...
        private static void CheckPassword(string password)
        {
            if (string.IsNullOrEmpty(password))
                //throw new ArgumentException(Strings.New_Server I18N.GetString("Password can not be blank"));
                throw new ArgumentException();
        }
